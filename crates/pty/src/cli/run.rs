...
pub fn run(args: &[String]) -> CliResult {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--lifecycle-tag" => {
                lifecycle_tag = args.get(i + 1).cloned();
                i += 2;
            }
            "--tag" => {
    ...
    if startup_timeout_ms.is_some() != lifecycle_tag.is_some() {
        eprintln!("pty run: --startup-timeout-ms and --lifecycle-tag must be provided together.");
        return Ok(1);
    }
    ...
    params.startup_lease =
        startup_timeout_ms
            .zip(lifecycle_tag)
            .map(|(timeout_ms, lifecycle_tag)| StartupLeaseOptions {
                timeout_ms,
                lifecycle_tag,
            });
