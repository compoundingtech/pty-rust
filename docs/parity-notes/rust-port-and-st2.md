...
|---|---|---|
| 1 `run …` | **No** | `--unset-env`/`--env` are unparsed and become the command (`pty-rust/src/bin/pty.rs:122-127`); daemon exec fails, `pty run` still exits 0 and prints the id, no `<id>.json` is written → st2 sees no session and respawns every pass. Every other flag st2 passes (`-d --force --id --name --no-display-name --cwd --tag`) is accepted. The CoS note's working experiment therefore must have used a build beyond main@e4d6cda (or the node daemon's registry with a patched binary) — not verifiable from the repos. |
